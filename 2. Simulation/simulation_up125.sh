...
preffix=$(echo -e "$chr\t$preffix_start\t$preffix_end" | bedtools getfasta -fi GCF_014441545.1_ROS_Cfam_1.0_genomic.fna -bed - |tail -n +2)
str=$(awk -v copy="$copy" -v pattern="$pattern" 'BEGIN { for (i=1; i<=copy; i++) printf pattern }')
suffix=$(echo -e "$chr\t$suffix_start\t$suffix_end" | bedtools getfasta -fi GCF_014441545.1_ROS_Cfam_1.0_genomic.fna -bed - |tail -n +2)
# 生成文件并输出结果 
seed=$((RANDOM % 90000 + 10000))
echo ">$name-"X"$seed"X"-"P"$bias"
