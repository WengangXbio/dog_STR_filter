...
result=read.table("simulation.estimation_detection.results",head=F)
...
ggplot(dff, aes(x = len, y = gap, fill = mean)) +
  ...
  labs(title = "Detection rate", x = "STR length", y = "Copy difference") +  theme_bw()
ggsave("Detection_rate.png",dpi = 400, width = 8, height = 8)
